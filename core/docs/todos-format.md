...
When the directory is empty, there are no active TODOs. No sentinel file is needed.
...
| `ID` | The TODO identifier (see ID Scheme below) | `M-WRK-8` |
...
The filename is generated by `todoFilename()` in `core/todo-files.ts`. The sortable prefix means `ls` naturally orders files by priority then domain.

## File Content Format

Each todo file is a standalone markdown document:
...
Acceptance: <Conditions that make this TODO "done". Concrete, verifiable statements.>
...
```
**Test plan:**
- Add unit test for `parseTodoFile` with new field extraction
- Verify existing integration tests still pass (no regressions)
- Edge case: TODO file with missing domain field (should default to "uncategorized")
```
