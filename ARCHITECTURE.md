...
3. **Ephemeral secret overrides.** For CI runners, shared containers, and other environments where the secret should not land on disk, `NINTHWAVE_BROKER_SECRET` (env var) or `nw --broker-secret-stdin` (reads one line from stdin) supply the secret for a single session. Precedence, highest to lowest: `--broker-secret-stdin` > `NINTHWAVE_BROKER_SECRET` > `.ninthwave/config.local.json` > `.ninthwave/config.json`. There is deliberately no `--broker-secret <value>` flag -- argv would expose the secret to the process listing and shell history.
4. **Auto-connect at startup.** When any layer above supplies a `broker_secret`, `nw` auto-connects to the crew. `--local` opts out for one session; `--connect` forces auto-connect even when no secret is configured (for scripted onboarding). Without a secret and without `--connect`, the daemon runs in local-only mode. The connection choice is never persisted: every session recomputes it from secret presence and CLI flags.
5. **Ongoing management (`nw crew`).** `nw crew` (or `nw crew status`) reports whether a secret and `crew_url` are configured. `nw crew create` generates and saves a new secret (confirming before overwrite). `nw crew join <secret>` validates and saves a teammate-provided secret. `nw crew disconnect` removes the secret from `.ninthwave/config.local.json`.
