...
`,
    ...
    const output = captureOutput(() =>
      cmdConflicts(["H-SH-1", "H-SH-2"], todosFile, worktreeDir),
    );
  ...
  it("detects domain overlap between items", () => {
    ...
    const output = captureOutput(() =>
      cmdConflicts(["M-CI-1", "H-CI-2"], todosFile, worktreeDir),
    );
  ...
  it("cross-repo items don't conflict", () => {
    ...
    const output = captureOutput(() =>
      cmdConflicts(["H-API-1", "H-WA-1"], todosFile, worktreeDir),
    );
  ...
  it("same-repo items are still compared", () => {
    ...
    const output = captureOutput(() =>
      cmdConflicts(["H-API-1", "M-API-2"], todosFile, worktreeDir),
    );
  ...
  it("reports CLEAR when no conflicts found", () => {
    ...
    const output = captureOutput(() =>
      cmdConflicts(["M-CI-1", "C-UO-1"], todosFile, worktreeDir),
    );
  ...
  it("errors with fewer than 2 IDs", () => {
    ...
    const output = captureOutput(() =>
      cmdConflicts(["M-CI-1"], todosFile, worktreeDir),
    );
